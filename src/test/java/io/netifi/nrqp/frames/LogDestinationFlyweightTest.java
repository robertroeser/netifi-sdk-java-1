...
public class LogDestinationFlyweightTest {
  ...
  public void testEncodeDestination() {
    int length = LogDestinationFlyweight.computeLength(RouteType.CHANNEL_ID_ROUTE, 0);
    byte[] bytes = new byte[length];
    ByteBuf byteBuf = Unpooled.wrappedBuffer(bytes);
    int seqId = 100;
    int accountId = 1;
    int destinationId = 2;
    LogDestinationFlyweight.encodeRouteByDestination(
        byteBuf, 100, RouteType.CHANNEL_ID_ROUTE, 1, 2);
    ...
    Assert.assertEquals(RouteType.CHANNEL_ID_ROUTE, LogDestinationFlyweight.routeType(byteBuf));
  }
}
