...
import io.netifi.proteus.admin.frames.AdminRouterNodeInfoEventType;
              ...
              return rSocket
                  ...
                  .map(
                      result -> {
                        ...
                        String routerId = AdminRouterNodeInfoResultFlyweight.routerId(metadata);
                        String host = AdminRouterNodeInfoResultFlyweight.adminAddress(metadata);
                        int port = AdminRouterNodeInfoResultFlyweight.adminPort(metadata);

                        switch (type) {
                          case JOIN:
                            logger.debug("router with host {} and port {} joining", host, port);
                            return DiscoveryEvent.add(
                                routerId, InetSocketAddress.createUnresolved(host, port));
                          case LEAVE:
                            logger.debug("router with host {} and port {} leaving", host, port);
                            return DiscoveryEvent.remove(
                                routerId, InetSocketAddress.createUnresolved(host, port));
                          default:
                            throw new IllegalStateException("unknown router node info event type");
                        }
                      });
            });
  }
