// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: admin.proto

package io.netifi.proteus.admin.om;

public interface NodeOrBuilder
    extends
    // @@protoc_insertion_point(interface_extends:io.netifi.proteus.admin.om.Node)
    com.google.protobuf.MessageOrBuilder {

  /** <code>string renderer = 1;</code> */
  java.lang.String getRenderer();
  /** <code>string renderer = 1;</code> */
  com.google.protobuf.ByteString getRendererBytes();

  /** <code>string name = 2;</code> */
  java.lang.String getName();
  /** <code>string name = 2;</code> */
  com.google.protobuf.ByteString getNameBytes();

  /** <code>string entryNode = 3;</code> */
  java.lang.String getEntryNode();
  /** <code>string entryNode = 3;</code> */
  com.google.protobuf.ByteString getEntryNodeBytes();

  /** <code>double maxVolume = 4;</code> */
  double getMaxVolume();

  /** <code>string class = 5;</code> */
  java.lang.String getClass_();
  /** <code>string class = 5;</code> */
  com.google.protobuf.ByteString getClass_Bytes();

  /** <code>int64 updated = 6;</code> */
  long getUpdated();

  /** <code>repeated .io.netifi.proteus.admin.om.Node nodes = 7;</code> */
  java.util.List<io.netifi.proteus.admin.om.Node> getNodesList();
  /** <code>repeated .io.netifi.proteus.admin.om.Node nodes = 7;</code> */
  io.netifi.proteus.admin.om.Node getNodes(int index);
  /** <code>repeated .io.netifi.proteus.admin.om.Node nodes = 7;</code> */
  int getNodesCount();
  /** <code>repeated .io.netifi.proteus.admin.om.Node nodes = 7;</code> */
  java.util.List<? extends io.netifi.proteus.admin.om.NodeOrBuilder> getNodesOrBuilderList();
  /** <code>repeated .io.netifi.proteus.admin.om.Node nodes = 7;</code> */
  io.netifi.proteus.admin.om.NodeOrBuilder getNodesOrBuilder(int index);

  /** <code>repeated .io.netifi.proteus.admin.om.Connection connection = 8;</code> */
  java.util.List<io.netifi.proteus.admin.om.Connection> getConnectionList();
  /** <code>repeated .io.netifi.proteus.admin.om.Connection connection = 8;</code> */
  io.netifi.proteus.admin.om.Connection getConnection(int index);
  /** <code>repeated .io.netifi.proteus.admin.om.Connection connection = 8;</code> */
  int getConnectionCount();
  /** <code>repeated .io.netifi.proteus.admin.om.Connection connection = 8;</code> */
  java.util.List<? extends io.netifi.proteus.admin.om.ConnectionOrBuilder>
      getConnectionOrBuilderList();
  /** <code>repeated .io.netifi.proteus.admin.om.Connection connection = 8;</code> */
  io.netifi.proteus.admin.om.ConnectionOrBuilder getConnectionOrBuilder(int index);

  /** <code>string displayName = 9;</code> */
  java.lang.String getDisplayName();
  /** <code>string displayName = 9;</code> */
  com.google.protobuf.ByteString getDisplayNameBytes();

  /** <code>repeated string metadata = 10;</code> */
  java.util.List<java.lang.String> getMetadataList();
  /** <code>repeated string metadata = 10;</code> */
  int getMetadataCount();
  /** <code>repeated string metadata = 10;</code> */
  java.lang.String getMetadata(int index);
  /** <code>repeated string metadata = 10;</code> */
  com.google.protobuf.ByteString getMetadataBytes(int index);

  /** <code>.io.netifi.proteus.admin.om.Metrics metrics = 11;</code> */
  boolean hasMetrics();
  /** <code>.io.netifi.proteus.admin.om.Metrics metrics = 11;</code> */
  io.netifi.proteus.admin.om.Metrics getMetrics();
  /** <code>.io.netifi.proteus.admin.om.Metrics metrics = 11;</code> */
  io.netifi.proteus.admin.om.MetricsOrBuilder getMetricsOrBuilder();
}
