...
public abstract class LoadBalancedRSocketMono extends Mono<RSocket>
  ...
  public Mono<Void> close() {
    return Mono.fromRunnable(onClose::onComplete)
        .doFinally(
            s -> {
              synchronized (this) {
                factoryRefresher.dispose();
                activeFactories.clear();
                for (WeightedSocket w : activeSockets) {
                  w.close().subscribe();
                }
              }
            })
   ...
   * list.
   */
  private class FactoriesRefresher extends BaseSubscriber<Collection<RSocketSupplier>> {
    @Override
    public void hookOnNext(Collection<RSocketSupplier> newFactories) {
  ...
  private class SocketAdder implements Subscriber<RSocket> {
    ...
    public void onNext(RSocket rs) {
      synchronized (LoadBalancedRSocketMono.this) {
        ...
        logger.debug("Adding new WeightedSocket2 {}", weightedSocket);
  ...
  private class WeightedSocket extends RSocketProxy implements LoadBalancerSocketMetrics {
          ...
          .doOnError(
              t -> {
                ...
                if (t instanceof TransportException || t instanceof ClosedChannelException) {
                  removeSocket(WeightedSocket.this, true);
    ...
    public Flux<Payload> requestStream(Payload payload) {
      ...
      return source
          ...
          .doOnComplete(() -> pendingStreams.decrementAndGet());
    }
    ...
    public Mono<Void> fireAndForget(Payload payload) {
      ...
      return source
          ...
          .doOnSuccess(s -> pendingStreams.decrementAndGet());
    }
